...
{
    public:
        StatsWindow(std::size_t width, std::size_t height, std::string name, const Stats& stats_current, const Stats& stats_global) :
            Window(width, height, name), m_stats_current(stats_current), m_stats_global(stats_global)
        {
            ...
            textSurface = TTF_RenderText_Blended_Wrapped(font, stats_global.to_string().c_str(), Definitions::WHITE_COLOR, width);
